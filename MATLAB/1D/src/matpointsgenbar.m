function [x, approximation, matpoints ] = matpointsgenbar( grid, L, nmatpointspercell,rho,E,n,v0)
...
for i=1:nelements
    ...
    xlocal = gausspositions(nmatpointspercell);
    delement = grid.nodes(nodes(end)).pos - grid.nodes(nodes(1)).pos;
    
    for j=1:length(xlocal)
        N = grid.elements(i).N(xlocal(j));
        matpointposition = grid.nodes(nodes(1)).pos*N(1) + grid.nodes(nodes(2)).pos*N(2);
...
x = [matpoints.pos];
approximation = [matpoints.velocity];
end
